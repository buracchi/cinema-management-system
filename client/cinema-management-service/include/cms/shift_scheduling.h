...
struct cms_add_shift_response {struct cms_response; };
extern int cms_add_shift(cms_t cms, struct cms_add_shift_request request, struct cms_add_shift_response** response);

struct cms_delete_shift_request { int32_t employee_id; char day[CMS_DAY_LEN]; char start_time[CMS_TIME_LEN]; };
struct cms_delete_shift_response {struct cms_response; };

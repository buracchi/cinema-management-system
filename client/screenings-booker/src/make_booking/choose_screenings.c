...
static char* get_screenings_table(struct cms_get_cinema_screenings_response* response) {
	...
	for (uint64_t i = 0; i < response->num_elements; i++) {
		...
		try(snprintf(hall_id, INT32DSTR_LEN, "%d", response->result[i].hall_id) < 0, true, fail2);
		try(ft_write_ln(table, screening_id, date, time, hall_id, film_name, running_time, price) < 0, true, fail2);
	}
